...
function saveWaterGoal(goal) {
    localStorage.setItem("gym_water_goal", String(goal));
    renderWaterTracker();
}
function getWaterLogForToday() {
...
function loadCalorieParams() {
    const paramsStr = localStorage.getItem("gym_calorie_calculator_params");
    if (!paramsStr)
        return;
    try {
        const params = JSON.parse(paramsStr);
        const genderEl = document.getElementById("cal-gender");
        ...
        const goalEl = document.getElementById("cal-goal");
        if (genderEl)
            genderEl.value = params.gender;
        if (ageEl)
            ageEl.value = String(params.age);
        if (heightEl)
            heightEl.value = String(params.height);
        if (weightEl)
            weightEl.value = String(params.weight);
        if (activityEl)
            activityEl.value = String(params.activity);
        if (goalEl)
            goalEl.value = params.goal;
...
function calculateCalories(interactive = true) {
    const genderEl = document.getElementById("cal-gender");
    const ageEl = document.getElementById("cal-age");
    const heightEl = document.getElementById("cal-height");
    const weightEl = document.getElementById("cal-weight");
    ...
    const params = { gender, age, height, weight, activity, goal };
    ...
    if (calValEl)
        calValEl.textContent = `${caloriesRounded} ккал`;
    if (protValEl)
        protValEl.textContent = `${proteinG} г`;
    if (fatValEl)
        fatValEl.textContent = `${fatG} г`;
    if (carbValEl)
        carbValEl.textContent = `${carbG} г`;
    if (protPctEl)
        protPctEl.textContent = `${Math.round(proteinPct * 100)}%`;
    if (fatPctEl)
        fatPctEl.textContent = `${Math.round(fatPct * 100)}%`;
    if (carbPctEl)
        carbPctEl.textContent = `${Math.round(carbPct * 100)}%`;
    if (resultsDiv)
...
export { saveBodyWeight, renderBodyStats, renderBodyChart, addWater, resetWater, renderWaterTracker, saveWaterGoal, loadCalorieParams, calculateCalories, };
