...
function openSettingsModal() {
    ...
    const dataEncrypted = isEncrypted(localStorage.getItem("trainingProgress") || "");
    const encryptionOn = hasPassphrase || dataEncrypted;
    if (encryptToggle) {
        encryptToggle.checked = encryptionOn;
        encryptToggle.onchange = () => {
            if (encryptInput)
                encryptInput.disabled = !encryptToggle.checked;
        };
    }
    if (encryptInput) {
...
async function saveSettings() {
    ...
    if (encryptToggle?.checked) {
        ...
        if (dataEncrypted && (!passphrase || passphrase === "********")) {
            showToast("Введіть поточний пароль для вимкнення шифрування", "warning");
            if (encryptInput) {
