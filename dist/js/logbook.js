...
const LogbookModule = {
    ...
    bindEvents() {
        ...
        if (btnSave) {
            btnSave.addEventListener("click", (e) => {
                ...
                if (!exerciseId || this.activeSets.length === 0)
                    return;
        ...
        if (exSearch) {
            exSearch.addEventListener("input", () => {
                this.filterSelect("logbook-ex-select", exSearch.value);
            });
        }
        const histSearch = document.getElementById("logbook-history-search");
        if (histSearch) {
            histSearch.addEventListener("input", () => {
                this.filterSelect("logbook-history-select", histSearch.value);
            });
        }
    ...
    deleteExercise() {
        ...
        if (confirm(`Ви впевнені, що хочете видалити вправу "${ex.name}"? Її минулі записи в історії збережуться.`)) {
            customExercises.splice(customExercises.findIndex((e) => String(e.id) === String(exId)), 1);
