...
import { vibrate, safeJSONParse, calculate1RM } from "./utils.js";
...
async function syncToCloud() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert("Помилка синхронізації: " + (err.message || `HTTP ${response.status}`));
    }
  } catch (e) {
...
async function fetchFromCloud() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert("Не вдалося завантажити дані (HTTP " + response.status + ")");
    }
  } catch (e) {
    alert("Помилка мережі: " + e.message);
  }
